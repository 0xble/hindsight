...
# This module seeds every case with `_insert_fact`, an INSERT into `memory_units` (plus `memory_links` / `unit_entities`). A MEMORIES extension owns those rows in its own store and leaves the Postgres
