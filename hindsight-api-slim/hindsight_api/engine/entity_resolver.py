...
class EntityResolver:
    ...
    async def record_unit_entity_postings(
        ...
        The two combine: with ``store_write=False`` there is no store write left for ``txn`` to tag,
        because the write that carried the entities inline is already in the group.
        """
        if not unit_entity_pairs:
            return
        ...
        return await self._link_units_to_entities_batch_impl(
            None, normalized, bank_id, txn=txn, store_write=store_write
        )
