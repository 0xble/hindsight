...
| `HINDSIGHT_API_LLM_LANGUAGE_INTEGRITY` | Source-language guard for retain facts and consolidation observations: `off`, `observe`, `retry`, or `reject`. `observe` emits metrics only; `retry` adds generic language-preservation guidance and one corrective regeneration but accepts a persistent mismatch; `reject` fails it after the correction without consuming source facts. Disabled when `HINDSIGHT_API_LLM_OUTPUT_LANGUAGE` is set. | `observe` |
...
| `HINDSIGHT_API_CONSOLIDATION_MAX_CONTEXT_TOKENS` | Hard input-token budget for one consolidation LLM call. The local preflight uses this to split oversized prompts before sending them to the provider. Keep below the provider context limit. Configurable per bank. | `100000` |
...
| `HINDSIGHT_API_CONSOLIDATION_FAIR_GROUP_SELECTION` | Fetch the oldest facts of many observation-scope groups per consolidation round instead of the oldest facts overall. Each group contributes at most one lane's share of the fetch (`ceil(batch size / CONSOLIDATION_LLM_PARALLELISM)`), visited in order of its oldest fact, so one large group (typically the shared scope) cannot fill every fetch and leave the parallel lanes idle. Order within a group stays oldest-first and same-scope batches still run serially under the per-scope locks. Applies to unscoped consolidation jobs on Postgres; other stores and scoped jobs keep the strict fetch. Configurable per bank. | `false` |
