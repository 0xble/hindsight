...
| `HINDSIGHT_API_RETAIN_CHUNKS_MODE_BATCH_TOKENS` | The same budget, used when `retain_extraction_mode` is `chunks`. That mode stores each chunk as-is and never calls an LLM, so the context-sized default above only limits how many chunks the streaming pipeline can work on at once. The higher budget lets a long document go through in one pass, so its chunks are embedded and written concurrently instead of a couple of dozen at a time. Lower it if a retain worker needs a smaller memory ceiling. | `500000` |
